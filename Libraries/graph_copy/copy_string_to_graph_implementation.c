...
# endif
				...
				if (arity>1){
					if (arity<256){
						...
						++n_words;
#endif
						n_free_words-=n_words;
						if (n_free_words<0){
							*last_heap_pa=heap_p+n_words+1+(stack_end-stack_begin);
							return (Int*)((Int)&string_p[1]+1);
						}
...
#ifdef PROFILE_GRAPH
						*heap_p++=(Int)profile_current_cost_centre;
#endif
...
#ifdef PROFILE_GRAPH
						non_pointers_p[n_non_pointers]=(Int)profile_current_cost_centre;
#endif
						...
						heap_p[1]=string_p[1];
#ifdef PROFILE_GRAPH
						heap_p[2]=(Int)profile_current_cost_centre;
#endif
						string_p+=2;
						heap_p+=3;
						break;
					...
					if (arity==0){
#ifdef PROFILE_GRAPH
						heap_p[1]=(Int)profile_current_cost_centre;
#endif
						heap_p+=3;
						break;
					} else {
